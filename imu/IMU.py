...
def detectIMU():
    ...
    try:
        ...
        LSM9DS0_WHO_G_response = (
            bus.read_byte_data(
                LSM9DS0_GYR_ADDRESS,
                LSM9DS0_WHO_AM_I_G))
        LSM9DS0_WHO_XM_response = (
            bus.read_byte_data(
                LSM9DS0_ACC_ADDRESS,
                LSM9DS0_WHO_AM_I_XM))
    except IOError as e:
        ...
        if (LSM9DS0_WHO_G_response == 0xd4) and (
                LSM9DS0_WHO_XM_response == 0x49):
    ...
    try:
        ...
        # This section of code stops this from happening.
        LSM6DSL_WHO_AM_I_response = (
            bus.read_byte_data(
                LSM6DSL_ADDRESS,
                LSM6DSL_WHO_AM_I))
        LIS3MDL_WHO_AM_I_response = (
            bus.read_byte_data(
                LIS3MDL_ADDRESS,
                LIS3MDL_WHO_AM_I))
...
def initIMU():
    ...
    if(BerryIMUversion == 1):  # For BerryIMUv1
        ...
        writeByte(LSM9DS0_ACC_ADDRESS, LSM9DS0_CTRL_REG1_XM, 0b01100111)
        writeByte(
            LSM9DS0_ACC_ADDRESS,
            LSM9DS0_CTRL_REG2_XM,
            0b00011000)  # +/- 8G full scale

        # initialise the magnetometer
        writeByte(LSM9DS0_MAG_ADDRESS, LSM9DS0_CTRL_REG5_XM,
                  0b11110000)  # Temp enable, M data rate = 50Hz
        writeByte(
            LSM9DS0_MAG_ADDRESS,
            LSM9DS0_CTRL_REG6_XM,
            0b01100000)  # +/- 12gauss
        writeByte(LSM9DS0_MAG_ADDRESS, LSM9DS0_CTRL_REG7_XM,
                  0b00000000)  # Continuous-conversion mode

        # initialise the gyroscope
        # Normal power mode, all axes enabled
        writeByte(LSM9DS0_GYR_ADDRESS, LSM9DS0_CTRL_REG1_G, 0b00001111)
        # Continuos update, 2000 dps full scale
        writeByte(LSM9DS0_GYR_ADDRESS, LSM9DS0_CTRL_REG4_G, 0b00110000)

    elif(BerryIMUversion == 2):  # For BerryIMUv2
        # initialise the accelerometer
        # z, y, x axis enabled for accelerometer
        writeByte(LSM9DS1_ACC_ADDRESS, LSM9DS1_CTRL_REG5_XL, 0b00111000)
        writeByte(
            LSM9DS1_ACC_ADDRESS,
        ...
        # initialise the gyroscope
        writeByte(LSM9DS1_GYR_ADDRESS, LSM9DS1_CTRL_REG4,
                  0b00111000)  # z, y, x axis enabled for gyro
        writeByte(LSM9DS1_GYR_ADDRESS, LSM9DS1_CTRL_REG1_G,
                  0b10111000)  # Gyro ODR = 476Hz, 2000 dps
        writeByte(
            LSM9DS1_GYR_ADDRESS,
            LSM9DS1_ORIENT_CFG_G,
            0b10111000)  # Swap orientation

        # initialise the magnetometer
        # Temp compensation enabled,Low power mode mode,80Hz ODR
        writeByte(LSM9DS1_MAG_ADDRESS, LSM9DS1_CTRL_REG1_M, 0b10011100)
        writeByte(
            LSM9DS1_MAG_ADDRESS,
            LSM9DS1_CTRL_REG2_M,
            0b01000000)  # +/- 2gauss
        writeByte(
            LSM9DS1_MAG_ADDRESS,
            LSM9DS1_CTRL_REG3_M,
            0b00000000)  # continuos update
        writeByte(LSM9DS1_MAG_ADDRESS, LSM9DS1_CTRL_REG4_M,
                  0b00000000)  # lower power mode for Z axis

    elif(BerryIMUversion == 3):  # For BerryIMUv3
        # initialise the accelerometer
        # ODR 3.33 kHz, +/- 8g , BW = 400hz
        ...
        writeByte(
            LSM6DSL_ADDRESS,
            LSM6DSL_CTRL2_G,
            0b10011100)  # ODR 3.3 kHz, 2000 dps

        # initialise the magnetometer
        # Temp sesnor enabled, High performance, ODR 80 Hz, FAST ODR disabled
        # and Selft test disabled.
