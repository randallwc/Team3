...
while True:
    ...
    if isIdle:
        outputString += 'idle\t'
    if isForwardTilt:
        outputString += 'tilting forward\t'
    if isForwardPush:
        outputString += 'forward push\t'
    if isUpwardLift:
        outputString += 'upward lift\t'
    ...
    str_yG = str(yG)
    str_zG = str(zG)
    str_gyroXangle = str(gyroXangle)
    str_gyroYangle = str(gyroYangle)
    str_gyroZangle = str(gyroZangle)

    #lines = [str_xG, str_yG, str_zG]
    with open("back_tilt.csv", "a") as file:
        file.writelines(
            str_xG +
            ',' +
            str_yG +
