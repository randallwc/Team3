...
while True:
    ...
    # This needs to be taken into consideration when performing the
    # calculations
    ...
    if 1:  # Change to '0' to stop  showing the angles from the Kalman filter
        outputString += "# kalmanX %5.2f   kalmanY %5.2f #" % (
            kalmanX, kalmanY)

    # IDLE
    threshold = 1
    kVals += [(kalmanX + kalmanY) / 2]
    kVals.pop(0)
    diffIdle = abs(kVals[0] - kVals[-1])
    isIdle = diffIdle < threshold

    # https://github.com/ozzmaker/BerryIMU/blob/master/python-BerryIMU-measure-G/berryIMU-measure-G.py#L36-L39
    yG = (ACCx * 0.244) / 1000
    xG = (ACCy * 0.244) / 1000
    zG = (ACCz * 0.244) / 1000
    outputString += "##### X = %fG  ##### Y =   %fG  ##### Z =  %fG  #####  " % (
        yG, xG, zG)

    # code for tilt recognition
    isForwardPush = yG < -0.08
    isUpwardLift = zG > 1.1
    #isForwardTilt = 0 < gyroXangle and gyroXangle < 70

    #isForwardTilt = xG < 0.75 and xG > 0.55 and yG < 0.2 and yG > -0.05 and zG > 0.6 and zG < 0.8
    #isBackwardTilt = xG < -0.5 and xG > -0.75 and yG < 0.2 and yG > -0.05 and zG > 0.6 and zG < 0.8
    ...
    # if isForwardPush:
        #tiltString += 'forward push\t'
    # if isUpwardLift:
     #   tiltString += 'upward lift\t'
    ...
    # Send the tilt direction
    message = str.encode(tiltString)
